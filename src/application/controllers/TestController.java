...
public class TestController extends Controller {
	...
	public void initialize() throws Exception {
		...
		PreparedStatement preparedStatement = Database.connection
				.prepareStatement("SELECT * FROM surgeries WHERE ID = ?");
...
//							.createField(permissions.get(healthCondition.healthCondition.columnName)).connectedTo(ubg),
//					0, gridPane.getChildren().size());
//			gridPane.add(healthCondition.notes.createField(permissions.get(healthCondition.patientID.columnName))
//					.connectedTo(ubg), 0, gridPane.getChildren().size());
//			gridPane.add(healthCondition.severity.createField(true).connectedTo(ubg), 0, gridPane.getChildren().size());
//			gridPane.add(healthCondition.creationTime.createField(true).connectedTo(ubg), 0,
...
//		}
		...
		// gridPane.add(surgery.doctorID.createField(true), 0,
		// gridPane.getChildren().size());
		// gridPane.add(surgery.location.createField(true), 0,
		// gridPane.getChildren().size());
		...
		PreparedStatement preparedStatement2 = Database.connection
				.prepareStatement("SELECT * FROM healthConditions WHERE ID = 1");
		ResultSet resultSet2 = preparedStatement2.executeQuery();
		resultSet2.next();

		// HealthCondition healthCondition = new HealthCondition(resultSet2);

		// ValueField healthConditionField = new
		// ValueField(healthCondition.healthCondition, true);
		// gridPane.add(healthConditionField, 0, 1);

		// ValueField healthConditionField2 = new ValueField(healthCondition.notes,
		// false);
