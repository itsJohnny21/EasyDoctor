...
SELECT employees.ID, sex, phone, managerID, lastName, firstName, email, birthDate, address FROM employees JOIN users ON employees.ID = users.ID WHERE users.role = 'DOCTOR';
select * from patients;